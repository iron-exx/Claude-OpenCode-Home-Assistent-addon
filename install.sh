...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .hdr-btn { background:none; border:1px solid var(--border); color:var(--text-muted); border-radius:8px; padding:4px 10px; font-size:12px; cursor:pointer; transition:all .15s; }
    ...
    .field-row input,.field-row select { background:var(--bg); border:1px solid var(--border); border-radius:8px; color:var(--text); padding:8px 10px; font-size:13px; outline:none; transition:border-color .15s; }
    ...
    .session-item { padding:8px 10px; border-radius:8px; cursor:pointer; border:1px solid transparent; transition:all .15s; }
    .session-item:hover { background:var(--surface2); border-color:var(--border); }
    .session-item.active { background:rgba(88,101,242,.1); border-color:rgba(88,101,242,.3); }
    ...
    .session-date { font-size:10px; color:var(--text-muted); }
    .session-provider { font-size:10px; padding:1px 5px; border-radius:4px; }
    .session-provider.anthropic { background:rgba(217,119,87,.15); color:var(--claude); }
    .session-provider.opencode  { background:rgba(34,197,94,.15);  color:var(--opencode); }
    .session-delete { opacity:0; font-size:11px; color:var(--text-muted); background:none; border:none; cursor:pointer; padding:2px 4px; border-radius:4px; transition:all .15s; }
    .session-item:hover .session-delete { opacity:1; }
    .session-delete:hover { color:var(--red); background:rgba(248,113,113,.1); }
    .sidebar-empty { text-align:center; padding:24px 16px; color:var(--text-muted); font-size:12px; }
    ...
    .typing-bubble { color:var(--text-muted); font-size:13px; padding:4px 0; }
    ...
    #send-btn:hover { background:var(--accent2); } #send-btn:disabled { background:var(--border); cursor:not-allowed; }
...
<div id="settings-panel">
  <div>
    ...
    <div class="provider-tabs">
      <div class="provider-tab anthropic active" id="tab-anthropic" onclick="selectProvider('anthropic')">
        <div class="tab-icon">🤖</div><div class="tab-name">Anthropic Claude</div><div class="tab-desc">API-Key erforderlich</div>
      </div>
      <div class="provider-tab opencode" id="tab-opencode" onclick="selectProvider('opencode')">
        <div class="tab-icon">🥒</div><div class="tab-name">OpenCode Big Pickle</div><div class="tab-desc">Kostenlos · Lokal</div>
      </div>
    </div>
  </div>
  ...
  <div class="provider-fields" id="fields-opencode">
    <div class="field-row">
      ...
      <div class="field-hint">PC: <code>opencode serve --hostname 0.0.0.0 --port 4096</code> · Modell: Big Pickle (automatisch)</div>
    </div>
  </div>
</div>
...
<div id="main">
  ...
  <div id="sidebar">
    <div class="sidebar-header">
      <span>Verlauf</span>
      <button id="new-chat-btn" onclick="newChat()">+ Neu</button>
    </div>
    <div id="sessions-list">
      <div class="sidebar-empty">Noch keine gespeicherten Chats</div>
    </div>
  </div>
  ...
  <div id="chat-area">
    <div id="messages">
      <div id="welcome">
        ...
        <p>Chats werden automatisch gespeichert und können links im Verlauf wieder geöffnet werden.</p>
      </div>
    </div>
    <div id="input-area">
      ...
      <div id="input-wrapper">
        <textarea id="user-input" placeholder="Nachricht schreiben..." rows="1"></textarea>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(lastId) {
    setTimeout(() => openSession(lastId), 300); // kurz warten bis DOM bereit
  }

  // Wenn Tab wieder sichtbar wird: Session neu laden
  document.addEventListener('visibilitychange', () => {
    if(document.visibilityState === 'visible') {
      ...
      if(sid && sid === currentSessionId) {
        // Nur neu laden wenn Nachrichten fehlen
  ...
  ['api-key-input','model-select','opencode-url-input'].forEach(id =>
  ...
  try {
    ...
    for(const msg of messageHistory) {
      if(msg.role === 'user' && typeof msg.content === 'string') {
        addMessage('user', msg.content, null, s.provider, false);
      } else if(msg.role === 'assistant' && typeof msg.content === 'string') {
        addMessage('assistant', msg.content, null, s.provider, false);
      }
    }
...
function newChat() {
  ...
  messageHistory = [];
  const msgs = document.getElementById('messages');
  msgs.innerHTML = `<div id="welcome"><div class="big-icon">🏠</div><h2>Neuer Chat</h2><p>Was kann ich für dich tun?</p></div>`;
  loadSessions();
}
...
function loadSettings() {
  ...
  selectProvider(localStorage.getItem('claude_provider') || 'anthropic');
}
...
function getSettings() {
  return {
    ...
    session_id: currentSessionId || (currentSessionId = crypto.randomUUID()),
  };
}
...
function addMessage(role, content, toolCalls, provider, scroll=true) {
...
function addTyping(provider) {
  ...
  avatar.textContent = provider==='opencode'?'🥒':'🤖';
  const bubble=document.createElement('div'); bubble.className='bubble';
  const label = provider==='opencode' ? '🥒 OpenCode denkt nach' : '🤖 Claude denkt nach';
  bubble.innerHTML=`<div class="typing-bubble">${label}...</div>`;
  wrap.appendChild(avatar); wrap.appendChild(bubble);
...
async function sendMessage() {
  ...
  const dotTimer = setInterval(()=>{
    ...
    const label = settings.provider==='opencode'?'🥒 OpenCode denkt nach':'🤖 Claude denkt nach';
    typingEl.querySelector('.typing-bubble').textContent = label+d;
  }, 700);
  ...
  try {
    ...
    if(startData.error) { clearInterval(dotTimer); typingEl.remove(); addMessage('assistant','❌ '+startData.error); return; }
    ...
    if(!result) { addMessage('assistant','⏱ Zeitüberschreitung – bitte nochmal versuchen.'); }
    else if(result.status==='error') { addMessage('assistant','❌ '+result.error); }
    else {
      addMessage('assistant', result.response, result.tool_calls, settings.provider);
      messageHistory = result.messages || messageHistory;
      if(!result.messages) messageHistory.push({role:'assistant',content:result.response});
      if(result.session_id) { currentSessionId = result.session_id; localStorage.setItem('last_session_id', currentSessionId); }
      loadSessions();
    }
  } catch(err) {
    clearInterval(dotTimer); typingEl.remove();
