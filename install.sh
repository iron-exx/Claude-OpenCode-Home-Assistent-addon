...
async function sendMessage() {
  ...
  try {
    ...
    if(startData.error) {
      ...
      addMessage('assistant', '❌ ' + startData.error);
      return;
    }

    // Schritt 2: Polling bis Antwort fertig
    ...
    const pollInterval = setInterval(async () => {
      ...
      if(typingEl) {
        ...
        const p = settings.provider === 'opencode' ? '🥒 OpenCode denkt nach' : '🤖 Claude denkt nach';
        typingEl.querySelector('.bubble').innerHTML = `<div style="color:var(--text-muted);font-size:13px">${p}${d}</div>`;
      }
    }, 800);

    let result = null;
    const maxWait = 300; // 5 Minuten max
    for(let i = 0; i < maxWait; i++) {
      await new Promise(r => setTimeout(r, 1000));
      try {
        const pollRes = await fetch(BASE + '/api/chat/poll/' + jobId);
    ...
    if(!result) {
      addMessage('assistant', '⏱ Zeitüberschreitung – Bitte nochmal versuchen.');
